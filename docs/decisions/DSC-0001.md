...
# Reason: This function gets the parameters of the wrapper models and stores
# them in a dictionary so that they can be reached later when the information
# is needed.
# Limitations: This function will only work with models which have intercept
# and coefficients; it will cause an error if the function is called on a model
...
# Date: 2024-11-09
# Decision: Usage of def id(self) getter function which is used to generate
# a unique id for the object on which this function is called.
# Status: Accepted
...
# inherit from the Artifact class, so we can reach objects with their unique
# ids.
# Limitations: The id might not guarantee uniqueness if the version and asset
...
# Class
# Status: Accepted
# Motivation: Lack of safely reaching the artifact arguments such as name for
# encapsulation.
# Reason: The getter functions allow the user to view information in a safe
# environment to prevent leakage and maintain the private nature of attributes.
# Limitations: None
# Alternatives: None
...
# Limitations: It only allows one type to work with the functions at a time.
# Alternatives: Union (if we want to allow multiple types).

# DSC-0005: Usage of np.argmax
# Date: 2024-11-09
# Decision: Using np.argmax in pipeline when categorical data is one-hot encoded
# Status: Accepted
# Motivation: To be able to use sklearn models (chosen for this project) which
# require ground truth in a 1D array.
# Reason: When the one-hot encoder is used for categorical data, it places each
# category into a separate column. This was not a 1D array as needed for
# calculations, so np.argmax is used to group each category in one column with
# specific labels for each category (0,0,0,1,1,2...), allowing calculations to
# be done without any dimension errors.
...
# Date: 2024-11-09
# Decision: Using is_numeric_type from pandas to check if a column is numeric
...
# work properly.
# Alternatives: Use a for loop to check each row in the column with "is_instance()".

# DSC-0007: Usage of st.session_state 
# Date: 2024-11-09
# Decision: Using st.session_state to initialize objects, so their information
# is stored during interactions.
# Status: Accepted
...
# st.session_state allows the app to keep data available during runtime, which
# is useful when we want to keep the object data reachable during functionality.
...
# for training based on that proportion.
...
# Date: 2024-11-09
# Decision: Create a function to let users upload a CSV file and make predictions
